...
def validate_url_ip(url: str) -> None:
    ...
    # Only allow HTTP and HTTPS to prevent file://, gopher://, etc. Checked before
    # the hostname so schemes like file:/// (which have no hostname) are rejected
    # for the real reason — the disallowed scheme — rather than "no hostname".
